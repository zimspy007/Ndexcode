...
// Ndexcode names that are hardcoded should start with '-' so that collisions can't happen with input code
...
public:
	...
	string getCpp(const string& pn); // will throw an error if the Ndexcode name doesn't exist
...
public:
	...
	void name(const string& in); // converts a Ndexcode name to a posibly different C++ name
