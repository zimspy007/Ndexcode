...
public:
	...
	void setInput(Namespace nsIn, bool dynamicIn, Type left, Type right)
	{
		if (inputHasBeenSet)
		{
			throw NdexcodeError("tried to set input on an AST node '"+getString()+"' more then once", INTERNAL_ERROR, getToken());
		}
	...
	{
		if (!inputHasBeenSet)
			throw NdexcodeError("tried to get inLeftType before input was set", INTERNAL_ERROR, getToken());
		return inLeftType;
	}
	...
	{
		if (!inputHasBeenSet)
			throw NdexcodeError("tried to get inRightType before input was set", INTERNAL_ERROR, getToken());
		return inRightType;
	}
	*/
	...
	Type getReturnType()
	{
		if (!returnType)
		{
			if (!inputHasBeenSet)
			{
				throw NdexcodeError("tried to get return type from AST node when input had not been set", INTERNAL_ERROR, getToken());
			}
		...
		if (!returnType)
		{
			throw NdexcodeError("AST node "+getString()+"failed to supply a return type", INTERNAL_ERROR);
		}
	...
	Action getAction()
	{
		if (!action)
		{
			if (!ns)
			{
				throw NdexcodeError("tried to get action from AST node when input had not been set", INTERNAL_ERROR, getToken());
			}
	...
	void dealWithConstants()
	{
		if (!ns)
		{
			throw NdexcodeError("tried to deal with constants before input was set", INTERNAL_ERROR, getToken());
		}
...
public:
	...
	void resolveAction()
	{
		...
		{
			throw NdexcodeError("AstVoid given non void input", INTERNAL_ERROR, getToken());
		}
...
public:
	...
	static AstNode make(AstNode leftTypeIn, AstNode rightTypeIn, AstNode returnTypeIn, AstNode bodyIn)
	{
		...
		{
			throw NdexcodeError("AstFuncBody made with function input nodes that are not types", INTERNAL_ERROR);
		}
...
public:
	...
	void resolveAction()
	{
		throw NdexcodeError("AstType::resolveAction called, which it shouldn't have been", INTERNAL_ERROR, getToken());
	}
};
...
public:
	...
	void resolveAction()
	{
		throw NdexcodeError("AstActionWrapper::resolveAction called, which it shouldn't have been", INTERNAL_ERROR);
	}
...
public:
	...
	void resolveAction() {throw NdexcodeError("AstWhatevToActionFactory::resolveAction called, wich should never happen", INTERNAL_ERROR);}
