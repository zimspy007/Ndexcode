...
public:
	...
	{
		throw NdexcodeError("tried to get the literal value of 'void;", INTERNAL_ERROR);
	}
...
public:
	...
	string getCompactString()
	{
		switch (primType)
		{
			...
			case BYTE: return "y";
			case INANI: return "i";
			case INOMBOLO: return "n";

			default:
				throw NdexcodeError("tried to make " + getString() + " compact", INTERNAL_ERROR);
		}
	}
	...
	string getCppLiteral(void * data, CppProgram * prog)
	{
		...
		switch (primType)
		{
			...
			case INANI: val=to_string(*(int*)data); break;
			case INOMBOLO: val=doubleToString(*(double*)data); break;
			/*{
			...
			default:
				throw NdexcodeError("tried to convert " + getString() + " to C++ code", INTERNAL_ERROR);
		}
	...
	size_t getSize()
	{
		switch (primType)
		{
			...
			case INANI: return sizeof(int);
			case INOMBOLO: return sizeof(double);

			default:
				throw NdexcodeError("tried to get size of " + getString(), INTERNAL_ERROR);
		}
	}
...
public:
	...
	Type actuallyIs(Type target)
	{
		if (target->getType()!=TUPLE)
		{
			...
			{
				throw NdexcodeError("actuallyIs called on tuple with target that is not tuple", INTERNAL_ERROR);
			}
		}
...
public:
	...
	string getCppLiteral(void * data, CppProgram * prog)
	{
		throw NdexcodeError("getCppLiteral called on whatev type, wich should not have happened", INTERNAL_ERROR);
	}

	size_t getSize()
	{
		throw NdexcodeError("getSize called on whatev type, wich should not have happened", INTERNAL_ERROR);
	}
	...
	Type getSubType()
	{
		throw NdexcodeError("getSubType called on whatev type, wich should not have happened", INTERNAL_ERROR);
	}
...
{
	throw NdexcodeError("getAllSubTypes called on type that was not a tuple", INTERNAL_ERROR);
}
...
const Type Inani = TypeBase::makeNewPrimitive(TypeBase::INANI);
const Type Inombolo = TypeBase::makeNewPrimitive(TypeBase::INOMBOLO);
Type Amabala = nullptr;
...
string TypeBase::getString(PrimitiveType in)
{
	switch (in)
	{
		...
		case INANI: return "INANI";
		case INOMBOLO: return "INOMBOLO";
...
Type TypeBase::actuallyIs(Type target)
{
	if (!matches(target))
	{
		throw NdexcodeError("actuallyIs called with type that doesn't match", INTERNAL_ERROR);
	}

	if (isWhatev())
	{
		throw NdexcodeError("actuallyIs not implemented properly for Whatev type", INTERNAL_ERROR);
	}
