...
void CppNameContainer::addPn(const string& pn, const string& cppNameHint)
{
	...
	{
		throw NdexcodeError("Tried to add '"+pn+"' as a pn name to a CppNameContainer but that pn name already exists", INTERNAL_ERROR);
	}
...
void CppNameContainer::reserveCpp(const string& cpp, bool ignoreCollisions)
{
	...
	{
		throw NdexcodeError("called CppNameContainer::reserveCpp with id '"+cpp+"', which already exists", INTERNAL_ERROR);
	}
...
string CppNameContainer::getCpp(const string& pn)
{
	...
	if (result==pnToCppMap.end())
	{
		if (parent)
			...
			throw NdexcodeError("could not find C++ equivalent of '"+pn+"' in CppNameContainer::getCppForPn", INTERNAL_ERROR);
	}
	else
...
void CppFuncBase::endln()
{
	if (exprLevel>0)
	{
		throw NdexcodeError("non zero expression level when ending line in C++ program, code so far:\n"+indentString(source), INTERNAL_ERROR);
	}
...
void CppFuncBase::popExpr()
{
	if (exprLevel<=0)
	{
		throw NdexcodeError("CppProgram::popExpression called with zero expressionLevel", INTERNAL_ERROR);
	}
...
void CppFuncBase::pushBlock()
{
	if (exprLevel>0)
	{
		throw NdexcodeError("CppProgram::pushBlock called when expressionLevel was not zero", INTERNAL_ERROR);
	}
...
void CppFuncBase::popBlock()
{

	{
		if (blockLevel<=0)
		{
			throw NdexcodeError("CppProgram::popBlock called with zero indentationLevel", INTERNAL_ERROR);
		}
...
{
	globalTopCode+="// this C++ code is transpiled from Ndexcode\n";
	globalTopCode+="// Ndexcode v"+to_string(VERSION_X)+"."+to_string(VERSION_Y)+"."+to_string(VERSION_Z)+" was used\n";
...
string CppProgram::getTypeCode(Type in)
{
	...
	default:
		throw NdexcodeError("CppProgram::getTypeCode called with invalid type "+(TypeBase::getString(in->getType())), INTERNAL_ERROR);
	}
}
...
void CppProgram::addFunc(const string& name, vector<std::pair<string, string>> args, string returnType, string contents)
{
	if (hasFunc(name))
	{
		throw NdexcodeError("called CppProgram::addFunc with function name '"+name+"', which already exists", INTERNAL_ERROR);
	}
...
void CppProgram::pushFunc(const string& name, const string& cppNameHint, Type leftIn, Type rightIn, Type returnType)
{
	if (hasFunc(name))
	{
		throw NdexcodeError("called CppProgram::pushFunc with function name '"+name+"', which already exists", INTERNAL_ERROR);
	}
...
void CppProgram::popFunc()
{
	...
	{
		throw NdexcodeError("called CppProgram::popFunc when function wasn't ready", INTERNAL_ERROR);
	}
	...
	if (funcStack.empty())
	{
		throw NdexcodeError("called CppProgram::popFunc too many times", INTERNAL_ERROR);
	}
