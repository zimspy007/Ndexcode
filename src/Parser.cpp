...
int skipBrace(const vector<Token>& tokens, int start)
{
	...
	{
		throw NdexcodeError(FUNC + " called with index that is not a valid brace", INTERNAL_ERROR, tokens[start]);
	}
	...
	while(true)
	{
		...
		{
			throw NdexcodeError("no matching brace", SOURCE_ERROR, tokens[start]);
		}
...
/*AstNode parseExpression(const vector<Token>& tokens, int left, int right)
{
	...
	if (left>right)
	{
		throw NdexcodeError(FUNC + " sent left higher then right", INTERNAL_ERROR, tokens[left]);
	}
	else if (left==right)
	{
		...
		if (op)
		{
			if (op==ops->openPeren || op==ops->openSqBrac || op==ops->openCrBrac)
			{
				...
				if (j==left && i==right)
				{
					...
					{
						throw NdexcodeError("unknown bracket '"+op->getText()+"'", INTERNAL_ERROR);
					}
				}
			}
			else
	...
	throw NdexcodeError("could not find where to split expression '" + stringFromTokens(tokens, left, right) + "'", SOURCE_ERROR, tokens[left]);
...
int findExpressionSplit(const vector<Token>& tokens, int left, int right)
{
	...
	if (indexOfMin<0)
	{
		throw NdexcodeError(FUNC+" could not find operator to split expression", INTERNAL_ERROR, tokens[left]);
	}
...
AstNode parseExpression(const vector<Token>& tokens, int left, int right)
{
	...
	if (ops->isOpenBrac(tokens[left]->getOp()) && skipBrace(tokens, left)==right) // if the braces enclose this entire expression
	{
		...
		{
			throw NdexcodeError("unhandled brace '"+tokens[left]->getOp()->getText()+"'", INTERNAL_ERROR, tokens[left]);
		}
	}
	...
	{
		throw NdexcodeError("improper use of '"+op->getText()+"' operator", SOURCE_ERROR, tokens[i]);
	}

	if (op==ops->pipe)
	{
		throw NdexcodeError("invalid use of '"+op->getText()+"'", SOURCE_ERROR, tokens[i]);
	}
	...
	else if (op==ops->doubleColon)
	{
		...
		{
			throw NdexcodeError("you can only use constant assignment on a single identifier", SOURCE_ERROR, tokens[i]);
		}
	...
	{
		throw NdexcodeError("++ and -- are not yet implemented", SOURCE_ERROR, tokens[i]);
...
void parseTokenList(const vector<Token>& tokens, int left, int right, vector<AstNode>& nodes)
{
	...
	for (int i=left; i<=right; i++)
	{
		if (tokens[i]->getOp()==ops->import)
		{
			...
			{
				throw NdexcodeError("'"+ops->import->getText()+"' must be followed by a string literal", SOURCE_ERROR, tokens[i]);
			}
...
unique_ptr<AstType> parseType(const vector<Token>& tokens, int left, int right)
{
	...
	while (left<=right)
	{
		...
		if (left+2<=right && tokens[left+1]->getOp()==ops->colon)
		{
			...
			{
				throw NdexcodeError("identifier must be to the left of ':' in type", SOURCE_ERROR, tokens[left]);
			}
			...
			else if (tokens[left+2]->getOp()==ops->openCrBrac)
			{
				...
				if (j>right)
				{
					throw NdexcodeError(FUNC+" skipping brance went outside of range", INTERNAL_ERROR, tokens[left+1]);
				}
			...
			{
				throw NdexcodeError("invalid thingy '"+tokens[left+2]->getText()+"' in type", SOURCE_ERROR, tokens[left+2]);
			}
...
void importFile(vector<AstNode>& nodes, string path)
{
	...
	{
		throw NdexcodeError("file '"+path+"' failed to open or was empty", SOURCE_ERROR);
	}
