...
void AstList::resolveAction()
{
	...
	{
		throw NdexcodeError("AstList given non void input", INTERNAL_ERROR, getToken());
	}
...
void AstExpression::resolveAction()
{
	if (!inLeftType->isVoid() || !inRightType->isVoid())
	{
		throw NdexcodeError("AstExpression given non void input", INTERNAL_ERROR, getToken());
	}

	if (rightIn->isType())
	{
		throw NdexcodeError("types must be declared as constants", SOURCE_ERROR, rightIn->getToken());
	}
	else if (center->isType() || center->isFunctionWithOutput() || leftIn->isType())
	{
		throw NdexcodeError("a function implementation got into an expression node somehow", INTERNAL_ERROR, center->getToken());
	}
	else
	{
...
void AstConstExpression::resolveConstant()
{
	if (!inLeftType->isVoid() || !inRightType->isVoid())
	{
		throw NdexcodeError("AstConstExpression given non void input", INTERNAL_ERROR, getToken());
	}
...
void AstOpWithInput::resolveAction()
{
	if (token->getOp()==ops->ifOp)
	{
		...
		if (leftIn.empty())
		{
			throw NdexcodeError("'?' must have a conditional to its left", SOURCE_ERROR, token);
		}
		else if (leftIn.size()!=1)
		{
			throw NdexcodeError("'?' can only have one conditional to its left", SOURCE_ERROR, token);
		}
		...
		if (rightIn.empty())
		{
			throw NdexcodeError("'?' must have a statement to its right", SOURCE_ERROR, token);
		}
		...
		{
			throw NdexcodeError("'?' can only have 1 or 2 '|' seporated expressions to its right", SOURCE_ERROR, token);
		}
	}
	else if (token->getOp()==ops->loop)
	{
		...
		if (rightIn.size()>1)
		{
			throw NdexcodeError("'@' followed by multiple expressions", SOURCE_ERROR, token);
		}

		if (leftIn.size()==0)
		{
			throw NdexcodeError("condition needed before '@'", SOURCE_ERROR, token);
		}
		else if (leftIn.size()==1)
		{
		...
		{
			throw NdexcodeError("chain of length "+to_string(leftIn.size())+"preceding '@', it should be length 1-3", SOURCE_ERROR, token);
		}
	...
	else if (token->getOp()==ops->andOp || token->getOp()==ops->orOp)
	{
		...
		{
			throw NdexcodeError("'"+token->getOp()->getText()+"' can not be sent '|' separated sequence", SOURCE_ERROR, getToken());
		}

		if (leftIn.size()!=1 || rightIn.size()!=1)
		{
			throw NdexcodeError("'"+token->getOp()->getText()+"' must be given a left and right input", SOURCE_ERROR, getToken());
		}
		...
		{
			throw NdexcodeError("'"+token->getOp()->getText()+"' can only be used with Bools", SOURCE_ERROR, leftIn[0]->getToken());
		}

		if (rightAction->getReturnType()!=Bool)
		{
			throw NdexcodeError("'"+token->getOp()->getText()+"' can only be used with Bools", SOURCE_ERROR, rightIn[0]->getToken());
		}
	...
	{
		throw NdexcodeError("AstOpWithInput::resolveAction called for token '"+token->getOp()->getText()+"', which it shouldn't have been", INTERNAL_ERROR, token);
	}
	else
	{
		throw NdexcodeError("AstOpWithInput made with bad token '"+token->getText()+"'", INTERNAL_ERROR, token);
	}
}
...
void AstToken::resolveAction()
{
	...
	if (token->getType()==TokenData::IDENTIFIER || token->getType()==TokenData::OPERATOR)
	{
		...
		{
			throw NdexcodeError("non overloadable operator in AstToken, it should have been removed and processed by the parser", INTERNAL_ERROR, token);
		}
		...
		if (inLeftType->getType()==TypeBase::TUPLE && inLeftType->getSubType(token->getText()).type!=nullptr)
		{
			...
			{
				throw NdexcodeError("sorry, Ndexcode does not yet support mutating tuples", SOURCE_ERROR, token);
			}
		}
		*/
		...
		catch (IdNotFoundError err)
		{
			...
			{
				throw NdexcodeError("unknown overload for "+inLeftType->getString()+" "+token->getText()+" "+inRightType->getString()+"'", SOURCE_ERROR, token);
			}
			...
			{
				throw NdexcodeError("metatype handeling in "+FUNC+" not yet implemented", INTERNAL_ERROR, token);
			}
			else if (inRightType->isVoid())
			{
				//throw NdexcodeError("unknown identifier '"+token->getText()+"' (var can not be made bc right in type is "+type->getString()+")", SOURCE_ERROR, token);
				throw NdexcodeError("unknown identifier '"+token->getText()+"'", SOURCE_ERROR, token);
			}
			else if (!inRightType->isCreatable())
			{
				throw NdexcodeError("cannot create variable '"+token->getText()+"' of type {"+inRightType->getString()+"}", SOURCE_ERROR, token);
			}
			...
			{
				throw err.toNdexcodeError(token);
			}
		}
		*/
	}
	else if (token->getType()==TokenData::LITERAL || token->getType()==TokenData::STRING_LITERAL)
	{
		if (!inLeftType->isVoid() || !inRightType->isVoid())
		{
			throw NdexcodeError("a literal can not be given an input", SOURCE_ERROR, token);
		}
	...
	{
		throw NdexcodeError("AstToken givin invalid token '"+token->getText()+"' of type "+TokenData::typeToString(token->getType()), INTERNAL_ERROR, token);
	}
}
