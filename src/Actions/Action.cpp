...
ActionData::ActionData(Type returnTypeIn, Type inLeftTypeIn, Type inRightTypeIn)
{
	...
	{
		throw NdexcodeError("ActionData created with null type", INTERNAL_ERROR);
	}
}
...
public:
			...
			: ActionData(returnTypeIn, inLeftTypeIn, inRightTypeIn)
	{
		...
		if (lambdaIn==nullptr)
		{
			...
			{
				throw NdexcodeError("action '"+textIn+"' has not yet been implemented for the interpreter", RUNTIME_ERROR);
			};
		}
		else
		{
