...
public:
		...
		ActionData(actionIn->getReturnType(), stackFameIn->getLeftInType(), stackFameIn->getRightInType())
	{
		...
		{
			throw NdexcodeError(action->getDescription() + " put into function even though its inputs are not void", INTERNAL_ERROR);
		}
	}
	...
	void resolveAction()
	{
		if (!node || action)
		{
			throw NdexcodeError("FunctionAction::resolveAction called when this action is in the wrong state", INTERNAL_ERROR);
		}
		...
		if (!returnType->isVoid() && !returnType->matches(action->getReturnType()))
		{
			...
			//throw NdexcodeError("function body returns "+action->getReturnType()->getString()+" instead of "+returnType->getString(), SOURCE_ERROR, node->getToken());
			throw NdexcodeError("function body returns "+action->getReturnType()->getString()+" instead of "+returnType->getString()+"\n"+action->getDescription(), SOURCE_ERROR, node->getToken());
		}
	...
	void addToProg(Action inLeft, Action inRight, CppProgram* prog)
	{
		...
		// construct a unique name that will NEVER collide with another funtion (even in the case of overloaded functions with the same Ndexcode name)
