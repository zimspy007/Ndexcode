...
public:
	...
	{
		if (!actionIn)
			throw NdexcodeError(string() + "branch action created sent null action", INTERNAL_ERROR);

		if (!leftInputIn)
			throw NdexcodeError(string() + "branch action created sent null leftInput", INTERNAL_ERROR);

		if (!rightInputIn)
			throw NdexcodeError(string() + "branch action created sent null rightInput", INTERNAL_ERROR);
		...
		{
			throw NdexcodeError(leftInput->getDescription() + " put into branch even though its inputs are not void", INTERNAL_ERROR);
		}

		if (!rightInput->getInLeftType()->matches(Void) || !rightInput->getInRightType()->matches(Void))
		{
			throw NdexcodeError(rightInput->getDescription() + " put into branch even though its inputs are not void", INTERNAL_ERROR);
		}

		if (!leftInput->getReturnType()->matches(action->getInLeftType()))
		{
			throw NdexcodeError(leftInput->getDescription() + " return type is not the same as the left input of " + action->getDescription(), INTERNAL_ERROR);
		}

		if (!rightInput->getReturnType()->matches(action->getInRightType()))
		{
			throw NdexcodeError(rightInput->getDescription() + " return type is not the same as the right input of " + action->getDescription(), INTERNAL_ERROR);
		}
	}
