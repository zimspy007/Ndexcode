...
public:
		...
		:ActionData(Bool, Void, Void)
	{
		...
		{
			throw NdexcodeError("AndAction created with first action that does not return Bool", INTERNAL_ERROR);
		}

		if (secondAction->getReturnType()!=Bool)
		{
			throw NdexcodeError("AndAction created with second action that does not return Bool", INTERNAL_ERROR);
		}
	}
...
public:
		...
		:ActionData(Bool, Void, Void)
	{
		...
		if (firstAction->getReturnType()!=Bool)
		{
			throw NdexcodeError("OrAction created with first action that does not return Bool", INTERNAL_ERROR);
		}

		if (secondAction->getReturnType()!=Bool)
		{
			throw NdexcodeError("OrAction created with second action that does not return Bool", INTERNAL_ERROR);
		}
	}
