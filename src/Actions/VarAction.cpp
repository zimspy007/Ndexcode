...
public:
	...
	void* execute(void* inLeft, void* inRight)
	{
		if (!(*stackPtrPtr))
		{
			throw NdexcodeError("something fucked up big time. VarGetAction::execute called while stack pointer is still null", RUNTIME_ERROR);
		}
...
public:
	...
	void* execute(void* left, void* right)
	{
		if (!(*stackPtrPtr))
		{
			throw NdexcodeError("something fucked up big time. VarSetAction::execute called while stack pointer is still null", RUNTIME_ERROR);
		}
	...
	void addToProg(Action inLeft, Action inRight, CppProgram* prog)
	{
		...
		if ((nameHint=="me" || nameHint=="in") && getReturnType()->getType()==TypeBase::TUPLE)
		{
			...
			{
				throw NdexcodeError("can not set 'in' or 'me' inside expression in C++ because of some really dumb reason", INTERNAL_ERROR);
			}
...
public:
	...
	void addToProg(Action inLeft, Action inRight, CppProgram* prog)
	{
		if (getReturnType()==Amabala)
		{
			//addToProgPnStr(prog);
			//prog->name("$pnStr");
			prog->code(prog->getTypeCode(Amabala));
			prog->pushExpr();

				auto sizeInfo=getReturnType()->getSubType("_size");
				auto dataInfo=getReturnType()->getSubType("_data");
				if (sizeInfo.type!=Inani || dataInfo.type!=Byte->getPtr())
				{
					throw NdexcodeError("ConstGetAction::addToProg failed to access string properties", INTERNAL_ERROR);
				}

				prog->code(Inani->getCppLiteral((char*)data+sizeInfo.offset, prog));
				prog->code(", ");
				//prog->code(Inani->getCppLiteral((char*)data+sizeInfo.offset, prog));
