...
public:
		...
		ActionData(returnType, Void, Void)
	{
		...
		{
			throw NdexcodeError("CppCastAction was only designed to cast matching tuples, which is not how it is being used", INTERNAL_ERROR);
		}
	...
	void* execute(void* inLeft, void* inRight)
	{
		throw NdexcodeError("CppCastAction was executed in the interpreter, which shouldn't happen", INTERNAL_ERROR);
	}
...
Action getElemFromTupleAction(Type source, string name)
{
	if (!source->getSubType(name).type)
		throw NdexcodeError("could not find '"+name+"' in "+source->getString(), SOURCE_ERROR);
