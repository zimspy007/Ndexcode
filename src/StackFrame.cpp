...
void StackFrame::addMember(Type in)
{
	if (!in->isCreatable())
	{
		throw NdexcodeError("tried to insert uncreatable type "+in->getString()+" into stack frame", INTERNAL_ERROR);
	}
...
void StackFrame::setInput(Type left, Type right)
{
	...
	else
	{
		...
		{
			throw NdexcodeError("stack frame left input set to "+left->getString()+", which is neither creatable nore a normal void", INTERNAL_ERROR);
		}
		...
		{
			throw NdexcodeError("stack frame right input set to "+right->getString()+", which is neither creatable nore a normal void", INTERNAL_ERROR);
		}
...
size_t StackFrame::getRightOffset()
{
	...
	{
		throw NdexcodeError("tried to get the right input offset from a stack frame that does not have a right input", INTERNAL_ERROR);
	}
}
