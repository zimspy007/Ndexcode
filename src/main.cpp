...
struct Flags
{
	string myPath;					// path to the ndexcode exeutable that is now running
...
int main(int argc, char ** argv)
{
	...
	if (flags.flagError)
	{
		cout << "try 'ndexcode -h' for help" << endl;
		return 0;
	}

	if (flags.help)
	{
		cout << "Ndexcode v" << VERSION_X << "." << VERSION_Y << "." << VERSION_Z << endl;
		cout << "usage: ndexcode [options] [source file] [options]" << endl;
		cout << "options: " << endl;
		cout << "-v, -version      display the version of Ndexcode" << endl;
	...
	if (flags.version)
	{
		cout << "Ndexcode version " << VERSION_X << "." << VERSION_Y << "." << VERSION_Z << endl;
		return 0;
	}

	NdexcodeProgram program;
	...
	{
		cout << "multiple source files specified, Ndexcode does not currently support this" << endl;
		cout << "try 'ndexcode -h' for help" << endl;
		return 0;
	}
		...
		else
		{
			...
			if (!flags.binOutFile.empty() || flags.runCompiled)
			{
				...
				if (flags.runCompiled)
				{
					...
					cmd = "./"+binFileName + " --running-from-ndexcode " + str::join(cmdLineArgs, " ", false);
