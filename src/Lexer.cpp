		...
		if (newType!=type)
		{
			if (!tokenTxt.empty())
			{
				...
				else
				{
					...
					{
						NdexcodeError("invalid token '"+tokenTxt+"'", SOURCE_ERROR, token).log();
					}
					else
					{
		...
		if (newType!=TokenData::WHITESPACE && newType!=TokenData::LINE_END)
		{
			if (newType==TokenData::STRING_LITERAL && (*file)[i]=='\\')
			{
				...
				if ((*file)[i]=='n')
					...
					throw NdexcodeError(string()+"invalid escape character '\\"+(*file)[i]+"'", SOURCE_ERROR, makeToken(tokenTxt+(*file)[i], file, line, charPos-tokenTxt.size(), type));
			}
			else
			{
				tokenTxt+=(*file)[i];
