...
void NamespaceData::addNode(AstNode node, string id)
{
	...
	else if (id=="__destroy__")
	{
		...
		{
			throw NdexcodeError("incorrect type signiture for destroyer", SOURCE_ERROR, node->getToken());
		}
	...
	else if (id=="__copy__")
	{
		...
		{
			throw NdexcodeError("incorrect type signiture for copier", SOURCE_ERROR, node->getToken());
		}
...
Type NamespaceData::getType(string name, bool throwSourceError, Token tokenForError)
{
	...
	if (results.empty())
	{
		if (parent)
			...
			throw NdexcodeError("'"+name+"' type not found", SOURCE_ERROR);
		else
			return nullptr;
	}
	else if (results.size()!=1)
	{
		throw NdexcodeError("namespace has multiple defenitions of the same type '"+name+"'", INTERNAL_ERROR);
	}
	else if (results[0]->getReturnType()->getType()!=TypeBase::METATYPE)
	{
		throw NdexcodeError("node returning non meta type stored in namespace type map for type '"+name+"'", INTERNAL_ERROR);
	}
	else
	{
...
Action NamespaceData::getDestroyer(Type type)
{
	...
	{
		throw NdexcodeError("multiple destroyers for a single type in a single namespace", INTERNAL_ERROR);
	}
	else
	{
...
Action NamespaceData::getCopier(Type type)
{
	...
	{
		throw NdexcodeError("multiple copiers for a single type in a single namespace", INTERNAL_ERROR);
	}
	else
	{
		return nodes[0]->getAction();
...
Action NamespaceData::getActionForTokenWithInput(Token token, Type left, Type right, bool dynamic, bool throwSourceError, Token tokenForError)
{
	...
	if (!matches.empty())
	{
		...
		{
			throw NdexcodeError("multiple matching instances of '"+token->getText()+"' found", SOURCE_ERROR, tokenForError);
		}
		else
		{
			return nullptr;
	...
	if (!matches.empty())
	{
		...
		{
			throw NdexcodeError("multiple whatev instances of '"+token->getText()+"' found", SOURCE_ERROR, tokenForError);
		}
		else
		{
			return nullptr;
	...
	{
		if (foundNodes)
			throw NdexcodeError("correct overload of '"+token->getText()+"' not found for types "+left->getString()+" and "+right->getString(), SOURCE_ERROR, tokenForError);
		else
			throw NdexcodeError("'"+token->getText()+"' not found", SOURCE_ERROR, tokenForError);
	}
	else
	{
		return nullptr;
